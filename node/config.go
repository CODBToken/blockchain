...
import (
	...
	"github.com/yunhailanuxgk/go-jinbao/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of geth is "jinbao". If no
...
func (c *Config) NodeName() string {
	...
	if name == "jinbao" || name == "jinbao-testnet" {
		name = "jinbao"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "jinbao" instances.
...
func (c *Config) resolvePath(path string) string {
	...
	if c.name() == "jinbao" && isOldGethResource[path] {
		oldpath := ""
		if c.Name == "jinbao" {
...
func (c *Config) loadECDSAWithPwd() (*ecdsa.PrivateKey, error) {
	...
	overFunc := func() {
		fmt.Println("=================================================")
		fmt.Println(`> Must be first execute "jinbao security --unlock" `)
		fmt.Println("=================================================")
		os.Exit(-1)
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "JINBAO-keystore")
		ephemeral = keydir
	}
